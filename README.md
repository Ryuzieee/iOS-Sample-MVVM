...
- **UseCase**: `GetPokemonListUseCase`, `GetPokemonFullDetailUseCase`, `SearchPokemonUseCase`, `GetFavoritesUseCase`, `GetIsFavoriteUseCase`, `ToggleFavoriteUseCase`
...
- **Mapper**: `Data/Mapper/` フォルダに集約。`PokemonDetailMapper`, `PokemonSpeciesMapper`, `EvolutionChainMapper`, `AbilityMapper`, `PokemonNameMapper`
- **Repository Handler**: `handleWithCache` / `handleRemote` / `handleLocal` で例外を `AppError` に変換

#### `Core/UI`（共通 UI）
- **コンポーネント**: `UiStateContent`, `EmptyContent`, `ErrorContent`, `LoadingIndicator`, `PokemonGrid`, `PokemonImage`, `SearchTextField`, `ErrorDialog`, `PokemonTextComponents`, `AppIconButton`
- **文字列定数**: `Strings.swift` にアプリ全体の UI 文字列を画面ごとにグルーピング
- **ユーティリティ**: `UiState`（Idle / Loading / Success / Error）、`loadAsUiState` 関数

#### `Feature`（各画面）
- 各 ViewModel は `ObservableObject` を採用し `@Published` で状態管理
